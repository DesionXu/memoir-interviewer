...
# 签名构建需要真实 HOME（hvigor 从 ~/.ohos/config/material 读取解密密钥）；
# 无签名构建把缓存重定向到 /tmp 避免污染用户目录
export npm_config_cache="/tmp/hvigor-npm-cache"
mkdir -p "$npm_config_cache"
if [ ! -f "$PROJECT/signing.local.json5" ]; then
...
# 2) 注入本机签名配置（DevEco 加密格式密码，hvigor 官方流程才能解密）
if [ -f "$SIGNING" ]; then
...
print('✅ 已注入签名配置')
PYEOF
  TASK="assembleApp"    # 官方签名 + .app 打包（AGC 上架格式）
  HMODE="project"
else
  TASK="assembleHap"
  HMODE="module"
...
"$DEVECO/Contents/tools/node/bin/node" "$DEVECO/Contents/tools/hvigor/bin/hvigorw.js" \
  --mode "$HMODE" -p product=default -p buildMode="$MODE" "$TASK" --no-daemon
